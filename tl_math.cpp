...
LineIteration8Func ( void *refconPV,
                    ...
                    A_long numIter)
    {
        ...
        *bop_inP = reinterpret_cast<PF_Pixel8*>(inW.data)+ (inW.rowbytes* yL / sizeof(PF_Pixel));
        ...
        PF_Pixel *in02 = bop_inP + (inW.rowbytes  / sizeof(PF_Pixel)) - 1;//bottom left pixel in 3X3.
...
Render16(void  *refcon,
				...
				PF_EffectWorld  *outputP)
		{
			PF_Err				err = PF_Err_NONE;
			MathInfo		*miP = reinterpret_cast<MathInfo*>(refcon);
			PF_Pixel16			*bop_outP = reinterpret_cast<PF_Pixel16*>(outputP->data),
								*bop_inP = reinterpret_cast<PF_Pixel16*>(inputP->data);
			A_long  in_gutterL = (inputP->rowbytes / sizeof(PF_Pixel16)) - inputP->width,
				    out_gutterL = (outputP->rowbytes / sizeof(PF_Pixel16)) - outputP->width;
			...
			for (A_long yL = 0; yL < outputP->height; yL++) {
				...
				PF_Pixel16 *in00 = bop_inP - (inputP->rowbytes / sizeof(PF_Pixel16)) - 1;//top left pixel in 3X3.
				PF_Pixel16 *in10 = in00 + 1;//top middle pixel in 3X3.
				PF_Pixel16 *in20 = in10 + 1;//top right pixel in 3X3.
				PF_Pixel16 *in01 = bop_inP - 1;//mid left pixel in 3X3.
				PF_Pixel16 *in21 = bop_inP + 1;//top right pixel in 3X3.
				PF_Pixel16 *in02 = bop_inP + (inputP->rowbytes / sizeof(PF_Pixel16)) - 1;//bottom left pixel in 3X3.
				PF_Pixel16 *in12 = in02 + 1;//bottom middle pixel in 3X3.
				PF_Pixel16 *in22 = in12 + 1;//bottom right pixel in 3X3.
        ...
        PF_LayerDef		*outputP )
{
	...
	if (PF_WORLD_IS_DEEP(outputP)){
		Render16 ( (void*)&miP,in_data,inputP, outputP);
	}
    else {
		Render8((void*)&miP, in_data, inputP, outputP);
        /*miP.inW = *inputP;
        miP.outW = *outputP;
        A_long numThreads;

		
        ERR(suites.IterateSuite1()->AEGP_GetNumThreads(&numThreads));
		std::thread *tt = new std::thread[numThreads-1];
		A_long parts = inputP->height / (numThreads-1);
		for (A_long i = 1; i < numThreads-1; i++) {
			for (A_long pL = 0; pL < parts; pL++) {
				A_long yL = parts * (i - 1) + pL;
				tt[i] = std::thread(LineIteration8Func,(void*)&miP, i, pL, parts);
				// Check for interrupt!
				if ((yL) && (err = PF_PROGRESS(in_data, yL + 1, outputP->height))) {
					return err;
				}
		...
		delete[] tt;
       ERR(suites.IterateSuite1()->AEGP_IterateGeneric(inputP->height, (void*)&miP,LineIteration8Func));
        outputP = &miP.outW;*/
